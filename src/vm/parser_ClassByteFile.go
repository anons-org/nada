...
func (me *ClassByteFile) parserFloatInfo(idx uint8) {
	...
	o.tag = idx
	o.bytes = float64(Utils.ByteToFloat32(me.Read(4)))
	me.constanPool.Set(me.idx, o)
	me.idx++
}
...
func (me *ClassByteFile) print() {
	...
	me.constanPool.ForEach(func(e *Utils.MapEntry) {
		var s string = ""
		key, _ := e.GetKey().(int)
		if v, ok := e.GetVal().(*ConstantMethodref); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", key, "Methodref", v.classIdx, v.nameAndType)
		} else if _, ok := e.GetVal().(*ConstantStringInfo); ok {
			s = fmt.Sprintf("#%d = %s					", key, "String")
		} else if v, ok := e.GetVal().(*ConstantClassInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d", e.GetKey(), "Class", v.nameIdx)
		} else if v, ok := e.GetVal().(*ConstantInterfaceMethodRefInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", e.GetKey(), "InterfaceMethodref", v.itfClsIdx, v.nameAndTypeIdx)
		} else if v, ok := e.GetVal().(*ConstantNameAndType); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", e.GetKey(), "NameAndType", v.nameIdx, v.descIdx)
		} else if utf8, ok := e.GetVal().(*ConstantUtf8); ok {
			s = fmt.Sprintf("#%d = %s					%s", e.GetKey(), "Utf8", string(utf8.bytes))
		} else if intOb, ok := e.GetVal().(*ConstantIntegerInfo); ok {
			s = fmt.Sprintf("#%d = %s					%s", e.GetKey(), "Integer", int(intOb.bytes))
		} else if floatOb, ok := e.GetVal().(*ConstantFloatInfo); ok {
			s = fmt.Sprintf("#%d = %s					%s", e.GetKey(), "FLoat", float64(floatOb.bytes))
		} else if _, ok := e.GetVal().(*ConstantDoubleInfo); ok {
			s = fmt.Sprintf("#%d = %s					%s", e.GetKey(), "Double", 1)
		} else if _, ok := e.GetVal().(*ConstantLongInfo); ok {
			s = fmt.Sprintf("#%d = %s					%s", e.GetKey(), "Long", 1)
		} else if invOb, ok := e.GetVal().(*ConstantInvokeDynInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", e.GetKey(), "InvokeDynamic", invOb.bootMethodAttrIdx, invOb.nameAndTypeIdx)
		} else if invOb, ok := e.GetVal().(*ConstantFieldrefInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", e.GetKey(), "Fieldref", invOb.clsIdx, invOb.nameAndTypeIdx)
		} else if invOb, ok := e.GetVal().(*ConstantMethodHandleInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d.#%d", e.GetKey(), "MethodHandle ", invOb.refKind, invOb.refIdx)
		} else if invOb, ok := e.GetVal().(*ConstantMethodTypeInfo); ok {
			s = fmt.Sprintf("#%d = %s					#%d", e.GetKey(), "MethodType ", invOb.descIdx)
		}
		fmt.Println(s)
	})
...
func (me *ClassByteFile) bulidImmediate(method *FMethod, code []byte) {
	...
	data := Utils.NewBytes(code)
	n := 0
	for data.GetIdx() < len(code) {
		...
		case OP.BIPUSH: //nada:压入int到栈 jvm:压入uint8到栈
			v:=Utils.Byte8ToInt( data.Read(1) )
			method.addCode(n, op, NewFInt(int64(v)))
			n++
		case OP.SIPUSH:
			idx := binary.BigEndian.Uint16(data.Read(2))
			method.addCode(n, op, NewFInt(int64(idx)))
			n += 2
		case OP.ILOAD:
			idx := uint16(data.Read(1)[0])
			method.addCode(n, op, NewFUint16(idx))
...
func (me *ClassByteFile) getCtToIFObject(idx interface{}) IFObject {
	...
	case *ConstantLongInfo:
		hb := v.(*ConstantLongInfo).hByte
		lb := v.(*ConstantLongInfo).lByte
		hb = append(hb, lb...)
		//14位数字
		fVal := binary.BigEndian.Uint64(hb)
		v:=int64(fVal)
		ob = NewFInt(v)

	case  *ConstantFloatInfo:
		b:=v.(*ConstantFloatInfo).bytes
...
func (me *ClassByteFile) getMethodArgTypeInfo(buf string) IFObject {
	...
	arg		:=Utils.MiscIns().GetDescStrData(str0);
	ret		:=Utils.MiscIns().GetDescStrData(retArg);
